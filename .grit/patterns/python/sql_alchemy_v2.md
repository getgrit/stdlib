...
pattern convert_to_subquery() {
	`$var = select($args)` where {
		$program <: contains `select($query)` where { $query <: contains $var }
...
pattern select_list_to_vargs() { `select([$args])` => `select($args)` }

pattern session_scalars($selection, $stmt, $expr) {
	`session.scalars(select($selection).from_statement($stmt1)).all()` => `session.scalars(
    select($selection)
    .from_statement($stmt)
...
pattern select_options($var, $selection, $expr) {
	`$var = select($selection).options(with_expression($expr, $literal))` => `$var = select($selection, $literal.label("some_literal"))`
}
...
pattern migrate_pre_1_4_select() {
	...
	or {
		`select($args)`,
		`$table.select($args)` where { $call_chain += `$table` }
	} as $stmt where {
		or {
			$args <: contains list(),
...
pattern migrate_post_1_4_create_legacy_select() {
	...
	or {
		...
		`$table.create_legacy_select($args)` where { $call_chain += `$table` }
	} as $stmt where { $stmt <: migrate_old_select_args($call_chain, $args) }
}
