...
pattern by_containing_text() {
	bubble or {
		`$element(by.cssContainingText($css, $text))` where {
			$element <: pw_elements()
		},
...
pattern by_handler() {
	bubble or {
		...
		`by.$by($selector)`
	} where { $by <: change_by($selector, $locator) } => `page.locator($locator)`
}
	...
	file($body) where {
		...
		$body <: contains bubble or {
			...
			or {
				...
				arrow_function($body, $async) where {
					$async <: .
				} as $func => `async $func`,
				function($body),
				arrow_function($body),
				...
				$body <: contains bubble or {
					...
					`$element(by.$by($selector)).sendKeys($args)` where {
						$by <: maybe change_by($selector, $locator)
					} => `page.locator($locator).fill($args)`,
