---
title: Upgrade to Cloudflare v5 API
---
...
// Incomplete, needs more context on how api, err in v4 relates to client in v5
pattern convert_instantiator() {
    `$api, $err := $instantiator($api_key, $api_email)` where {
        $instantiator <: `cloudflare.New` => `cloudflare.NewClient`,
        $api_key <: `os.Getenv($key)` => `option.withAPIKey($api_key)`,
        $api_email <: `os.Getenv($email)` => `option.withAPIKey($api_email)`,
    }
}
...
file($body) where {
    // TODO: update when Go snippets can match package_identifier
    $body <: contains r"cloudflare",
    $body <: contains or {
        convert_instantiator(),
        convert_field_methods(),
    }
}
```
